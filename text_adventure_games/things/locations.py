...
class Location(Thing):
    ...
    def here(self, thing: Thing, describe_error: bool = True) -> bool:
        ...
        if not thing.location == self: # if the thing's location is not the same as the current location object then return False
            return False
        else:
            return True
    ...
    def add_item(self, item): # item will be an Item object
        """
        Put an item in this location.
        """
        self.items[item.name] = item # so item.name gives its name in string form and then the dictionary items helps us point to the item Object
        item.location = self
        item.owner = None
