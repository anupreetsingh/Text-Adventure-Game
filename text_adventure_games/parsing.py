...
import inspect
import textwrap # Module that helps you format plain text to fit within a given width
...
class Parser:
    ...
    def get_character(self, command: str) -> Character:
        ...
        If no names are matched, it returns the default value which in this case is the player.
        """
        command = command.lower()
    ...
    def get_items_in_scope(self, character=None) -> dict[str, Item]:
        ...
        items_in_scope = {} # items_in_scope is a dictionary that maps item_name to the item object
        # Goes through items in the current location
        for item_name in character.location.items: 
            items_in_scope[item_name] = character.location.items[item_name] 
        # Goes through items in the inventory
