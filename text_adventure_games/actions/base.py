...
class Action:
    ...
    def was_matched(
        ...
        """
        Checks to see if the thing was matched by self.parser.
        If the thing is None,it means it was not matched by the self.parser and it returns False along with the error message.
        If the thing is not None,it means it was matched by the self.parser and it returns True.
        """
        if thing is None:
            if not error_message:
